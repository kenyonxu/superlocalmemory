...
#:   REMOVED mcp/tools.py — flagged "wired" only by a stem-token collision:
#:          the sole "importer" is the fork's integrations/hermes/__init__.py
#:          importing the Hermes SDK `tools` package (tools.registry), not
#:          superlocalmemory.mcp.tools. The guard re-flags mcp/tools.py as dead
#:          if that import ever disappears, so the ratchet still functions.
#:
...
_KNOWN_DEAD: dict[str, str] = {
    ...
    "integrations/hermes/tests/conftest.py": "ALIVE — pytest fixture module; the fork "
        "ships the Hermes provider's tests in-src. Loaded by the test runner, "
        "never by production imports",
    "integrations/hermes/tests/test_provider.py": "ALIVE — in-src provider tests "
        "(fork layout); exercised by pytest, not imported by production code",
...
_KNOWN_DEAD_PACKAGES: dict[str, str] = {
    ...
    "integrations/hermes": "INTENTIONAL — fork-only Hermes MemoryProvider. The "
                  "Hermes plugin host loads it dynamically via plugin.yaml + "
                  "register(); a static import scan cannot see that entry point",
    "integrations/hermes/tests": "fork ships the provider's tests in-src; run by "
                  "pytest, never imported by production code",
