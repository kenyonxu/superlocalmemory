...
def test_v4_ports_mainline_migrations_under_unique_serials() -> None:
    ...
    assert "M042_correction_case_ledger" in eager_names
...
def test_schema_42_is_stamped_only_after_correction_storage_completes(tmp_path: Path) -> None:
    """A version-42 marker must prove correction ledger storage exists."""
    ...
    with sqlite3.connect(memory_db) as conn:
        assert conn.execute(
            "SELECT 1 FROM sqlite_master "
            "WHERE type='table' AND name='correction_cases'"
        ).fetchone() == (1,)
