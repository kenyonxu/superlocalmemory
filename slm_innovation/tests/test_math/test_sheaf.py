"""Tests for slm_innovation.math.sheaf — Sheaf cohomology contradiction detection.
...
from slm_innovation.math.sheaf import (
    ContradictionResult,
    SheafConsistencyChecker,
    TEMPORAL_RESTRICTION_SCALE,
    _restriction_for_edge_type,
    coboundary_norm,
    edge_residual,
)
from slm_innovation.storage.models import AtomicFact, EdgeType, GraphEdge
...
class TestRestrictionMap:
    ...
    def test_temporal_returns_scaled_identity(self) -> None:
        R = _restriction_for_edge_type("temporal", 3)
        expected = TEMPORAL_RESTRICTION_SCALE * np.eye(3)
...
class TestCoboundaryNorm:
    ...
    def test_temporal_scaling_reduces_severity(self) -> None:
        ...
        R_id = np.eye(3)
        R_temp = TEMPORAL_RESTRICTION_SCALE * np.eye(3)
