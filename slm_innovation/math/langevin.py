...
access-aware potential function.  Frequently accessed memories drift
toward the origin (ACTIVE, high retrieval weight), while neglected
memories diffuse toward the boundary (ARCHIVED, weight approaches 0).
...
from slm_innovation.storage.models import MemoryLifecycle
...
_ALPHA: float = 0.5   # Softer confinement toward origin
_BETA: float = 1.5    # Stronger access pull (log scale)
_GAMMA: float = 0.005 # Slower aging repulsion (per day)
_DELTA: float = 0.8   # Stronger importance attraction
...
class LangevinDynamics:
    ...
    def step(
        ...
        # --- Drift: -0.5 * lambda^{-2} * grad_U * dt ---
        drift = -0.5 * (lam_inv ** 2) * grad * self.dt

        # --- Geometric correction: 0.5 * T * (d-2) * lambda^{-1} * x * dt ---
        correction = 0.5 * self.temperature * (self.dim - 2) * lam_inv * xi * self.dt

        # --- Diffusion: sqrt(T * dt) * lambda^{-1} * noise ---
        rng = np.random.default_rng(seed)
        noise = rng.standard_normal(self.dim)
        diffusion = math.sqrt(self.temperature * self.dt) * lam_inv * noise
    ...
    def _potential_gradient(
        ...
        Where effective_alpha is modulated by metadata:
            effective_alpha = alpha
                - beta * log(access+1)/10   (frequent access → weaker confinement)
                + gamma * min(age, 365)/365  (aging → stronger confinement)
                - delta * importance          (important → weaker confinement)

        Gradient:
            grad U = 2 * effective_alpha * xi

        This ensures beta/gamma/delta ACTUALLY affect dynamics:
        - Frequently accessed facts drift further from origin (more accessible)
        - Old unused facts drift toward origin (fading)
        - Important facts resist confinement (persistent)
