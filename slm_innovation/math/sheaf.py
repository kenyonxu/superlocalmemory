...
if TYPE_CHECKING:
    ...
    from slm_innovation.storage.models import AtomicFact

logger = logging.getLogger(__name__)

_EPS = 1e-12

# Temporal edges use softer restriction (s*I) to tolerate legitimate
# state changes (job changes, moves, etc.) without false positives.
TEMPORAL_RESTRICTION_SCALE: float = 0.6
...
def _restriction_for_edge_type(
    ...
    Temporal edges use a softer scale (0.6*I) to tolerate legitimate
    state changes (job moves, relationship changes).
    """
    ...
    if edge_type == "temporal":
        return TEMPORAL_RESTRICTION_SCALE * np.eye(dim)
    return np.eye(dim)
