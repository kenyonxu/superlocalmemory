"""SLM Innovation — Temporal Parser (3-Date Model).
...
from slm_innovation.storage.models import AtomicFact, TemporalEvent
...
class TemporalParser:
    ...
    def __init__(self, reference_date: str | None = None) -> None:
        ...
        if reference_date is not None:
            try:
                self._ref = dateutil_parse(reference_date)
    ...
    def extract_dates_from_text(self, text: str) -> dict[str, str | None]:
        ...
        if dates_found:
            ...
            if len(dates_found) >= 2 and interval_start is None:
                sorted_dates = sorted(dates_found[:2])
