...
`mslm setup` 会自动下载嵌入模型（~500MB），缓存于 `~/.cache/huggingface/`。如果下载失败，见下方[网络与代理](#5-网络与代理)。
...
MSLM 可以按需启动（每次冷启动 ~3 秒），但推荐后台常驻：

```bash
mslm serve start     # 后台启动（端口 8765）
mslm serve status    # 查看状态
mslm serve stop      # 停止
mslm restart         # 一键重启（杀僵尸 + 清理 + 启动 + 健康检查）
```
...
Embed worker 子进程需要访问 `huggingface.co` 验证模型文件。在受限网络环境中，直连不可达，镜像站（如 `hf-mirror.com`）可能存在 SSL 兼容问题。MSLM 支持通过 `config.json` 配置代理。
...
```bash
mslm status          # 模式、profile、DB 大小
mslm health          # 数学层健康（Fisher-Rao、Sheaf、Langevin）
mslm list -n 20      # 最近 20 条记忆
mslm recall "查询"   # 语义检索
mslm dashboard       # Web 仪表盘（http://localhost:8765）
```

记忆整理：

```bash
mslm consolidate --cognitive   # 去重合并
mslm decay --execute           # 遗忘曲线衰减
mslm entity list               # 查看实体
```
...
```bash
# 一次性配置
pip install mslm-memory                        # 1. 安装
mslm setup                              # 2. 初始化（选 Mode A）
mslm config set proxy.http ...          # 3. 代理（如需）
mslm config set proxy.https ...
SLM_EMBED_RESPONSE_TIMEOUT=300 \        # 4. 启动 daemon
  mslm serve start

# 日常
mslm serve status                       # 确认在线
mslm dashboard                          # 可选：Web 面板
```

之后日常只需 `mslm serve start`，开机自启可配置 systemd service。

---

## 关于 MSLM

MSLM (Multi-Scope Local Memory) 是 SuperLocalMemory 的独立发行版，专注于多层次记忆协作场景。核心记忆引擎由 [SuperLocalMemory](https://github.com/qualixar/superlocalmemory) 驱动（AGPL-3.0）。

更多信息请参阅 [多层次结构记忆](multi-scope-memory-zh.md) 和 [文档索引](INDEX-zh.md)。
