...
# MSLM × Hermes Agent 集成指南

> 本指南面向 [Hermes Agent](https://github.com/3rdparty/hermes) 用户，介绍如何通过 MCP 协议将 MSLM 集成到 Hermes Agent 中，实现持久化多层次记忆。
...
> 也支持 `pip install superlocalmemory`，效果相同。CLI 命令 `mslm` 和 `slm` 等效。
...
## 3. Hermes Agent 接入 MSLM

MSLM 通过 MCP 协议对外提供服务。启动命令：
...
在 Hermes Agent 的 MCP 配置文件中添加 MSLM 服务。
...
> **关于数据目录**：MSLM 默认使用 `~/.superlocalmemory/` 存储所有数据。如需自定义路径，请确保 MCP 和 daemon（`mslm serve start`）使用**相同的** `SLM_DATA_DIR` 环境变量，否则记忆会写入两个不同的数据库。最简单的做法是不设置 `SLM_DATA_DIR`，使用默认路径。
...
MSLM 注册到 Hermes Agent 后，可直接调用以下工具：
...
MSLM 引入三层作用域记忆架构，让 Hermes Agent 团队中的多个 Agent 既能保持各自的记忆空间，又能共享公共知识。
...
> **注意**：当前版本 `include_global` / `include_shared` 参数已被接受（不再报错），但检索结果始终包含三层 scope 的记忆。精细的逐 scope 过滤将在后续版本中启用。
...
MSLM 支持多画像隔离，适合在 Hermes Agent 中切换不同工作上下文：
...
```bash
mslm serve start     # 后台运行
mslm serve status    # 查看状态
mslm serve stop      # 停止
```

或一键重启：
```bash
mslm restart         # 杀孤儿进程 + 清理 + 重启 + 健康检查
```
...
```bash
mslm dashboard       # 打开 http://localhost:8765
```
...
```bash
# 最常用
mslm remember "内容" --tags "标签1,标签2"
mslm remember "内容" --scope global               # 存入全局作用域
mslm remember "内容" --scope shared --shared-with "agent1,agent2"  # 指定共享
mslm recall "查询" --limit 10
mslm list -n 20
mslm status

# 维护
mslm consolidate --cognitive   # 认知整合（去重合并）
mslm decay --execute           # 遗忘曲线衰减
mslm quantize --execute        # 嵌入量化（节省空间）

# 实体管理
mslm entity list --scope personal   # 列出个人作用域的实体
mslm entity list --scope shared     # 列出共享作用域的实体
mslm entity list --scope global     # 列出全局作用域的实体
mslm entity merge <源ID> <目标ID>   # 合并重复实体（源被删除，目标保留）
...
| 现象 | 排查步骤 |
|------|---------|
| 冷启动慢 | 先运行 `mslm serve start` 启动守护进程 |
| 检索质量差 | 运行 `mslm health` 检查 Fisher-Rao / Sheaf 状态 |
| 工具调用失败 | 检查 `mslm doctor` 输出，确认 MCP 服务器正常 |
...
- [多层次结构记忆](multi-scope-memory-zh.md) — 三层作用域核心概念
- [快速上手指南](getting-started-zh.md) — 从安装到日常使用
- [配置指南](configuration-zh.md) — 运行模式与参数配置
