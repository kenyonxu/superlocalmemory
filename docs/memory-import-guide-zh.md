...
# MSLM 记忆导入指南：三层作用域批量导入

> 本指南介绍如何将外部记忆服务（其他 Agent 记忆、知识库、笔记系统等）的记忆批量导入 MSLM，并合理分配到三层作用域（personal / shared / global）。
...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `content` | 是 | 记忆内容文本，MSLM 会自动提取原子事实 |
...
1. **去重**：MSLM 的熵门控（entropy gate）会自动过滤高度相似的重复记忆，但仍建议导入前做基本去重
2. **速率控制**：批量导入时建议每 50 条暂停 0.5s，避免 pending queue 积压
3. **scope 选择**：不确定时默认用 `personal`，后续可通过 MCP 工具调整
4. **实体共享**：全局实体机制意味着导入 "React" 相关知识时，所有 Agent 自动共享 React 实体——无需手动同步
